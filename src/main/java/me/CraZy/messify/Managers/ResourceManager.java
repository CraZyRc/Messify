...
public class ResourceManager {
  ...
  private static void createLanguageFolder(File folder) {
    if (!folder.exists()) {
      try {
        ...
        Logger.info(ChatExpansion.Color("&4ERROR: Cannot create the language folder. Permission denied!"));
      }
    }
  }
  /**
  ...
  private static void loadFiles(String[] supportedLanguages) {
    ...
    for (String language : supportedLanguages) {
      ...
      try (var resource = Main.class.getClassLoader().getResourceAsStream( language + ".properties")) {
        if (resource == null) {
          Logger.info(ChatExpansion.Color(String.format("&4ERROR: Could not load in language resource %s!", language)));
          return;
        }
        file.createNewFile();
        ...
        languageLoadFailed = true;
        Logger.info(ChatExpansion.Color(String.format("&4ERROR: Could not instantiate language file %s!", file.toPath())));
      }
    }
  }
  ...
  private static void loadResource() {
    ...
    try (var Istream = new FileInputStream(langFile)) {
      ...
      languageLoadFailed = true;
      Logger.info(ChatExpansion.Color(String.format("&4ERROR: ailed to read from language file %s!", langFile.getPath())));
    }
  }
